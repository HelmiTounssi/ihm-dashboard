...
def plot_gauge(
    ...
    for ang, c in zip(ang_range, colors):
        ax.add_patch(
            Wedge((0.0, 0.0), 0.4, *ang, width=0.10, facecolor="w", lw=2, alpha=0.5)
        )
        ax.add_patch(
            Wedge((0.0, 0.0), 0.4, *ang, width=0.10, facecolor=c, lw=0, alpha=0.8)
        )
    ...
    for mid, lab in zip(mid_points, labels):
        ax.text(
            a * np.cos(np.radians(mid)),
            a * np.sin(np.radians(mid)),
            lab,
